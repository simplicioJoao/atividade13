...
import java.nio.charset.StandardCharsets; // Para usar a codificação UTF-8.
...
public class JwtTokenProvider {
    ...
    @Autowired
    private UserService userService; // Injeção do serviço UserService para buscar usuários.
    ...
    private Claims extractAllClaims(String token) {
        return Jwts.parserBuilder()
                .setSigningKey(secretKey.getBytes(StandardCharsets.UTF_8)) // Converte a chave secreta para bytes.
                .build()
    ...
    private String createToken(Map<String, Object> claims, String subject) {
        return Jwts.builder()
                ...
                .signWith(SignatureAlgorithm.HS256, secretKey.getBytes(StandardCharsets.UTF_8)) // Assinatura com algoritmo HS256 e chave secreta
    ...
    public String generateTokenById(int id) {
        // Alterado para chamar getUserById ao invés de findById
        ...
        UserDetails userDetails = User.withUsername(user.getUsername()) // Alterado para usar user.getUsername()
                .password(user.getPassword()) // Alterado para usar user.getPassword()
                .authorities("USER")
                .build();
